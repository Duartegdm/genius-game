...
int calcularDelay(int nivelAtual) {
  float fator = pow(0.90, nivelAtual); // razão pra aumentar a velocidade com o passar dos niveis    
...
void runTagSelect() {
  ...
  while (true) {
    ...
    if (btn == 1) { // Atualiza os caracteres da tag "simultaneamente"
...
void runGameOver() {
  ...
  lcdPrintStr(0, 1, "TAG:" + String(tag) + " P:" + String(pontos));
  ...
  // Tela de resultado
  lcdClear();
  lcdPrintStr(0, 0, "Pts:" + String(pontos) + " Nv:" + String(nivel + 1));
  delay(250);
  lcdPrint(0, 0, "Vm/Vd: Continuar");
  lcdPrint(0, 1, "Am/Az: Sair");
